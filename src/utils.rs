...
pub(crate) fn interp<T>(start: T, end: T, interp_amount: f64) -> T
where
    T: Sub<Output = T>,
    T: Add<Output = T>,
    T: Copy,
...
/// interpolation (with the `interp_amount` clamped between 0 and 1) for `(f64,f64)` types
pub(crate) fn interp2(start: (f64, f64), end: (f64, f64), interp_amount: f64) -> (f64, f64) {
    (
...
/// dot product for `(f46,f64)` types
pub(crate) fn dot(x: (f64, f64), y: (f64, f64)) -> f64 {
    x.0 * y.0 + x.1 * y.1
}

/// distance calculation for `(f64,f64)` types
pub fn dist(start: (f64, f64), end: (f64, f64)) -> f64 {
    ((start.0 - end.0).powi(2) + (start.1 - end.1).powi(2)).sqrt()
}

#[cfg(test)]
mod test_utils {
    use crate::utils::{interp, interp2, nearest_point_on_segment, normalize01_64};

    #[test]
    fn test_normalize_float() {
        approx::assert_relative_eq!(normalize01_64(1., 2., 1.5), 0.5);
        approx::assert_relative_eq!(normalize01_64(7., 3., 4.), 0.75); //should also work in reverse
        approx::assert_relative_eq!(normalize01_64(-1., 1., 2.), 1.);
        approx::assert_relative_eq!(normalize01_64(1., 1., 1.), 0.);
        approx::assert_relative_eq!(normalize01_64(1., 1., 0.), 0.);
        approx::assert_relative_eq!(normalize01_64(1., 1., 2.), 1.);
    }
